...
```ad-tip
title: Why are Container Runtimes so Confusing?
...
[[Docker]] 僅貢獻如何標準化的啟動 (run) 一個 container 給 [[Open Container Initiatives (OCI)]] 。[[Docker]] 並沒有將 image、或是 [[Registry]] push / pull 的格式貢獻出來。也就是説 [[Docker]] 僅將第三個步驟貢獻出來並標準化，而在這件事被釐清之前，所有人都認為 Container runtimes 幫我們完成了上面三個步驟，就像 [[Docker]] 過去所做的一樣。也造成 "Container runtimes" 這個字容易令人混淆的原因。
```
