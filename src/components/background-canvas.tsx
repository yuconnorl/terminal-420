...
import { BODIES_DIMENSION, COLOR_ARRAY, RATIO_CONSTANT, removeSensorOptions, WALL_THICKNESS } from '@/configs/matter'
...
const BackgroundCanvas = () => {
  ...
  function onPartyStart(number: number) {
    ...
    for (let i = 0; i < number; i++) {
      ...
      const weedBody = Bodies.rectangle(
        scene.current ? scene.current.clientWidth / 2 + i * 5 : window.innerWidth / 2,
        -i * 5,
  ...
  function onRemoveClick() {
    ...
    const ground = Composite.allComposites(engine.current.world)[0]?.bodies.find((body) => body.label === 'ground')
    ...
    Body.setPosition(
      ...
      Vector.create(
        scene.current ? scene.current.clientWidth / 2 : window.innerWidth / 2,
        scene.current ? scene.current.clientHeight + WALL_THICKNESS * 4000 : window.innerHeight + WALL_THICKNESS * 4000,
      ),
    )
  }
  ...
  const onMousemove = useCallback((e: MouseEvent) => {
    ...
    const centerX = scene.current ? scene.current.clientWidth / 2 : window.innerWidth
    const centerY = scene.current ? scene.current.clientHeight / 2 : window.innerHeight
  ...
  useEffect(() => {
    ...
    const rightWall = Bodies.rectangle(
      scene.current ? scene.current.clientWidth + WALL_THICKNESS / 2 : window.innerWidth + WALL_THICKNESS / 2,
    ...
    const ground = Bodies.rectangle(
      scene.current ? scene.current.clientWidth / 2 : window.innerWidth / 2,
      scene.current ? scene.current.clientHeight + WALL_THICKNESS / 2 : window.innerHeight + WALL_THICKNESS / 2,
      20882,
      WALL_THICKNESS,
      {
        isStatic: true,
    ...
    const removeSensor = Bodies.rectangle(
      scene.current ? scene.current.clientWidth / 2 : window.innerWidth / 2,
      scene.current ? scene.current.clientHeight + WALL_THICKNESS * 5 : window.innerHeight + WALL_THICKNESS * 5,
      10000,
      WALL_THICKNESS * 3,
      removeSensorOptions,
    )
    ...
    const removeSensorRight = Bodies.rectangle(
      scene.current ? scene.current.clientWidth + WALL_THICKNESS * 3 : window.innerWidth + WALL_THICKNESS * 3,
      scene.current ? scene.current.clientHeight / 2 : window.innerHeight / 2,
      WALL_THICKNESS * 3,
    ...
    Events.on(engine.current, 'collisionStart', (event) => {
      ...
      for (let i = 0, j = pairs.length; i != j; ++i) {
        ...
        if ((pair && pair.bodyA === removeSensor) || (pair && pair.bodyA === removeSensorRight)) {
          Composite.remove(engine.current.world, pair.bodyB)
        }

        if ((pair && pair.bodyB === removeSensor) || (pair && pair.bodyA === removeSensorRight)) {
    ...
    Composite.add(wallComposite, [ground, leftWall, rightWall, removeSensor, removeSensorRight])
    Composite.add(engine.current.world, wallComposite)

    function handleResize() {
      render.canvas.width = scene.current ? scene.current.clientWidth : window.innerWidth

      render.canvas.height = scene.current ? scene.current.clientHeight : window.innerHeight
    ...
    const createWordBody = (word: keyof typeof BODIES_DIMENSION, index: number, ratio: number) => {
    ...
    const words: Array<keyof typeof BODIES_DIMENSION> = ['hello', 'im', 'yu', 'from', 'taiwan', 'hand']
    ...
    Composite.add(engine.current.world, [developerBody, frontendBody, frontConstraint])
  ...
  return (
    <>
      <div
        ref={scene}
        className={clsx(
          'bg-galaxy absolute top-0 left-0 z-0 h-full w-full',
          isSpaceMode ? 'bg-cover bg-center bg-no-repeat' : 'bg-[length:0px_0px]',
        )}
      />
      <div className='absolute top-2 right-2 flex flex-col gap-3 md:top-4 md:right-4 md:gap-4'>
        <Tooltip title='Remove' icon='TrashBin' onClick={onRemoveClick} isSpaceMode />
        <Tooltip title='Space Mode' icon='OuterSpace' onClick={toggleOuterSpaceMode} isSpaceMode={isSpaceMode} />
        <Tooltip title='Party Time' icon='PartyRocket' onClick={() => onPartyStart(20)} isSpaceMode />
      </div>
    </>
  )
}
